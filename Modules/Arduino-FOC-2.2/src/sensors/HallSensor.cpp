 ...
 */
void HallSensor::attachSectorCallback(void (*_onSectorChange)(int sector)) { onSectorChange = _onSectorChange; }

float HallSensor::getSensorAngle() { return getAngle(); }
...
float HallSensor::getAngle() { return ((float)(electric_rotations * 6 + electric_sector) / (float)cpr) * _2PI; }

double HallSensor::getPreciseAngle() { return ((double)(electric_rotations * 6 + electric_sector) / (double)cpr) * (double)_2PI; }

int32_t HallSensor::getFullRotations() { return (int32_t)((electric_rotations * 6 + electric_sector) / cpr); }
